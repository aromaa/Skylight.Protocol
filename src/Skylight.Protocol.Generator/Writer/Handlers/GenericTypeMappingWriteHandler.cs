...
internal sealed class GenericTypeMappingWriteHandler : MappingWriterHandler
{
	...
	internal override void Write(ref WriterContext context, ProtocolStructure protocol, IndentedTextWriter writer, string? method, AbstractMappingSyntax mapping, MemberInfo type)
	{
		...
		if (genericMapping.Type == typeof(List<>).FromAssembly(genericMapping.Type) || genericMapping.Type == typeof(IAsyncEnumerator<>).FromAssembly(genericMapping.Type))
		{
			...
			bool recursive = genericMapping.GenericArgument is ObjectMappingSyntax objectMapping && protocol.Structures.TryGetValue(objectMapping.Type, out ObjectStructure? structure) && structure.Recursive;
			if (recursive)
			{
