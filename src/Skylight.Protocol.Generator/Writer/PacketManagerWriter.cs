...
internal static class PacketManagerWriter
{
	...
	internal static void Write(TextWriter stream, ProtocolStructure protocol)
	{
		...
		string packetHeaderType = protocol.Capabilities.Contains("STRING_PACKET_ID") ? "string" : "uint";
		...
		writer.WriteLine($"\t: AbstractGamePacketManager<{packetHeaderType}>(serviceProvider, baseData, GamePacketManager.GetProtocolData()), IGameProtocol");
		writer.WriteLine($"{{");
		writer.Indent++;
		...
		writer.WriteLine($"private static partial PacketManagerData<{packetHeaderType}> GetProtocolData();");
		writer.Indent--;
		writer.WriteLine($"}}");
	}
}
